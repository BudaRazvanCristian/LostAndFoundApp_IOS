...
type EditPostScreenProps = NativeStackScreenProps<{ EditPost: { item: Item } }, "EditPost">;
...
const EditPostScreen: React.FC<EditPostScreenProps> = ({ navigation, route }) => {
  ...
  const isOwner = Boolean(user?.id && item.userId && user.id === item.userId);
  ...
  const submitEdit = async () => {
    if (!isAuthenticated) {
      Alert.alert("Not authenticated", "You must log in to edit a post.");
      return;
    }

    if (!isOwner) {
      Alert.alert("Not authorized", "You can edit only your own post.");
      return;
    }
    ...
    try {
      await apiService.updatePost(item.id, {
        title: title.trim(),
        imageUri,
        status,
      ...
      await refreshItems();
      Alert.alert("Post updated", "Your post was updated successfully.");
      navigation.goBack();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      Alert.alert("Error updating post", errorMessage);
    } finally {
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Only the post owner can update this announcement.</Text>
          </View>

          <View style={styles.card}>
            <Text style={styles.sectionTitle}>Item photo</Text>
            <Text style={styles.sectionText}>Replace the current image or keep the existing one.</Text>
            ...
            <View style={styles.photoBox}>
              {imageUri ? (
                ...
                <View style={styles.photoPlaceholder}>
                  ...
                  <Text style={styles.photoPlaceholderText}>Add a clear image so the item is easy to identify.</Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.submitWrap}>
              <AppButton
                ...
                onPress={submitEdit}
                disabled={!canSubmit || isSubmitting || !isOwner}
              />
            </View>
          </View>
        </ScrollView>
